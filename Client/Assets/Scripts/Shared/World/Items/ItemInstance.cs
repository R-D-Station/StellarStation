namespace Shared.World.Items
{
    ...
    // GROUND-stream shape only; Placement определён ТОЛЬКО при Location==Ground, для Held/Inside = 0. Held НИКОГДА
    // не идёт через ItemInstance/WriteItem — он живёт как HeldItem в слоте владельца (InventorySync).
