...
namespace ServerTests.Server.Network
{
    ...
    public class IntentDrainTests
    {
        ...
        private static void ApplyClientIntent(ClientConnection c, GridMap map, MoveIntent intent, bool hasIntent)
        {
            ...
            {
                float x = c.X, y = c.Y;
                MovementLogic.Apply(map, c.Z, ref x, ref y, intent.Direction, intent.Sprint, crawl: c.State == PlayerState.Laying);
                c.X = x; c.Y = y;
        ...
        private static (int applied, int dropped) Drain(ClientConnection c, GridMap map)
        {
            int applied = 0;
            while (applied < MaxPerTick && c.IntentQueue.TryDequeue(out var intent))
            {
                ApplyClientIntent(c, map, intent, true);
                applied++;
            }

            int dropped = 0;
            if (applied == 0)
                ApplyClientIntent(c, map, default, false); // None-тик
            else
        ...
        {
            var map = new GridMap();
            for (int yy = 0; yy <= 50; yy++) map.SetTile(0, yy, 0, Tile.Floor()); // длинный проход на север
            return map;
        }
        ...
        public void Drain_BurstOfIntents_AllApplied_AckLast_PositionIsSum()
        {
            ...
            var (applied, dropped) = Drain(c, map);
        ...
        public void Drain_EmptyQueue_NoneTick_StandsNoMove()
        {
            ...
            var (applied, dropped) = Drain(c, Corridor());
